...
#[derive(Debug)]
pub struct PipeTracker {    //tracks number of pipes. doesnt let them go over 10.
    pipes_seen: usize,      //number of pipes currently rendered
    top: VecDeque<f32>,     //top of stack of pipes
    time: f32,              //makes sure player has enough time to jump to correct position after pipe
    random_fn: Perlin,      //adds a random amount of delay in next pipe spawn
}
...
impl PipeTracker {
    ...
    fn get_pipe_top(&mut self) -> f32 {     //pipe stack. gets next pipe to be spawned
        self.pipes_seen += 1;
    ...
    fn init_get_pipe_top(&mut self) -> f32 {    //sets next pipe to be spawned
    ...
    pub fn get_pipe_difference(&mut self) -> f32 {
        ...
        if self.pipes_seen == 10 {      //resetting when 10 pipes have been seen
            self.top.pop_front();
...
//pipe bottom is drawn before drawing pipe top and doing some calculations
//after pipe bottom is drawn we need to calculate the position of top pipe that leaves space for player to move through.
...
pub fn create_pipes(        //calls all the other pipe making functions
    sprite_base: Sprite,
    sprite_top: Sprite,
...
pub fn pipe_velocity() -> f32 {     //scrolling speed of pipes
    -PIPE_SPEED
}

fn make_pipe_body(sprite_base: &Sprite, x: f32, top: f32, total_dist: f32, top_height: f32, i: f32) -> Box<PipeEntity> {    //draws chain of pipe body (since pipe legnth may vary)
...
fn create_pipe_tip(sprite_top: Sprite, x: f32, top: f32, scroll : f32) -> PipeEntity {      //creates pipe tip which has a different sprite
