...
use audio::Player;
use crate::bird::{PlayerEntity, create_player};
...
pub struct GameState {
    ...
    pub pipe_tracker: PipeTracker,  //keeps track of pipes after they exit screen and reuses them
...
impl GameState {
    pub fn handle_after_losing(&mut self, ctx: &mut Context, state: PlayState) {    //auto restarts after player loses
        match state {
            PlayState::Dead { time } => {
    ...
    pub fn new(ctx: &mut Context, sprite_batch: SpriteBatch) -> Self {
        ...
        let atlas =
            atlas::Atlas::parse_atlas_json(std::path::Path::new("resources/texture_atlas.json"));   //creating sprites
